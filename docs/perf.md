...
| 経路 | シェル到達 | 実効速度 |
|---|---|---|
| bzImage | 971M命令 | ~36 MIPS (P1a後) |
| vmlinux 直接ロード | 580M命令 / **16.3s** | **35.6 MIPS** (P1a後。P1a前は21.7s/26.7) |
| wasm (headless) | 600M / 25.2s | 23.8 MIPS (P1a前は50〜66s — ~2倍) |
...
| # | 案 | 期待 | 状態 | メモ |
|---|---|---|---|---|
| B1 | デコード済み命令キャッシュ (P1a: 頻出族+フォールバック) | 2〜3倍 | ✅P1a済 | 交互A/B 21.7→16.3s (-25%)、wasm ~2倍 (50-66→25s)。カバレッジ93.7%、命令数不変。残り: フォールバック36Mの解析→P1b |
