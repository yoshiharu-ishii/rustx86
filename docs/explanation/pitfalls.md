...
## 14. 「置けた」は「使える」ではない — 足りないとき、機械は黙って半分だけ動く

**型**: 資源が足りないとき、素直に落ちてくれる相手は親切な方である。**足りない
なりに動いてしまう**相手は、成功したように見えて中身が欠ける。そして欠けた部分は、
**使おうとした人が最初の被害者**になる。

**事件**: gcc入りinitramfs (圧縮34MiB・展開後88MiB) を既定の128MBで起動したら、
`gcc hello.c` が `cannot execute 'cc1'` で止まった。イメージには焼いたはずの
`GCC_EXEC_PREFIX` も空。**イメージの中身を`strings`で見ると確かに入っている**
のに、ゲストの中には無い。カーネルはinitramfsをtmpfsへ展開するので、圧縮イメージと
展開後の中身がしばらく同時にRAMに載る。足りないと展開を途中でやめ、しかも
**"rootfs image is not initramfs (write error); looks like an initrd" と言って
先へ進む** — 「旧式のinitrdなんだろう」と解釈を変えてしまう。だから起動は成功し、
ルートFSだけが尻切れになる。

192MBという中途半端な量が一番たちが悪かった。欠けたのは `collect2`・`ld`・
udhcpcのスクリプトの4つだけで、**`gcc -c` は通るのにリンクだけ落ち、DHCPが
黙って効かない**機械になる。

**直し**: 起動前に算数で断る (`initrd_ram_needed`)。gzipなら展開後の大きさは
末尾4バイト (ISIZE) にそのまま書いてあるので、圧縮+展開後+カーネル作業域68MiBを
RAMと比べればよい。ランナー側は指定が無ければこの下限に25%足してRAMを自動で
決める。**ぴったりに寄せない** — 下限ちょうどの192MBが、まさに上の「4つだけ
欠ける」を引く量だった。

**波及**: gccイメージを継ぎ足し (mini + gcc の連結gzip) で作っていたら、ISIZEは
**最後の塊の分しか名乗らない**。少なく見積もって同じ穴に落ちるので、1本に詰め直す
ようにした (`make-gcc-initramfs.sh`)。道具に数字を聞くときは、その数字が何を
数えているかまで確かめる。
