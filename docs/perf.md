...
```
済み:    A1 A2(寝) B1 B2 B3(寝) B4 C1 C2 C4 C4b D2(寝) E1 E2 E6 — 13→78 MIPS
...
経過:   C1 -3〜4% (期待10〜25%はB4前の前提)、C4b -17% (memmove 11%の正体は
        Cpu丸ごと控えだった)、D2/TLB配列化は効かず
現在地: 素で**~78 MIPS** (C4b後7.4s/580M)。残り1.3倍。プロファイルは
        step_cached本体が64% — 次の一手はC3/D1の小物より**B5トレース化**が本命。
