...
| 経路 | シェル到達 | 実効MIPS |
|---|---|---|
| ネイティブ vmlinux (580M) | 9.0〜9.4s (交互A/B) | **~64** |
...
## 互換税の取り返し候補 (2026-08-12 追加)

| # | 案 | 見立て |
|---|---|---|
| S1 | **平坦セグメントの高速路**: base=0 & limit=4GB (Linuxの常態) をセグメントロード時に判定して隠しレジスタに控え、data_addr の検査と lin 再計算を1分岐で飛ばす | 互換税+10%の大半はここ。意味は不変 (検査が恒真のときだけ飛ばす) |
| S2 | flush_ad の分岐をdcacheループ外へ (tickと同じ「次の区切りで」方式) | 小さい。S1の後に交互A/Bで |
