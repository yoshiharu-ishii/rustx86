...
#### N3の層構造 — WebSocketに物理層を演じさせている

N3は「L2/L3より深い設計」ではない。むしろ**L2より下**にいる。
やっているのは、**WebSocketにL1 (物理層) の役をやらせる**ことである。

```mermaid
flowchart TB
    subgraph GUEST["① ブラウザ内のゲスト Linux"]
        direction TB
        APP["w3m / dig / ping"]
        L4G["TCP / UDP / ICMP"]
        L3G["IP"]
        L2G["Ethernet (virtio-netドライバ)"]
        APP --> L4G --> L3G --> L2G
    end

    subgraph WIRE["② ここ全部が「ケーブル」の代わり"]
        direction TB
        NIC["virtio-net (rustx86)"]
        WSK["WebSocket = L1 物理層の役"]
        TLSH["TLS"]
        L4H["TCP (ホストOS)"]
        L3H["IP → 実NIC"]
        NIC --> WSK --> TLSH --> L4H --> L3H
    end

    subgraph RELAY["③ 踏み台 EC2"]
        direction TB
        TERM["Caddy + リレー<br>WebSocketを剥がしてフレームを取り出す"]
        TAP["TAPデバイス"]
        FWD["ip_forward + MASQUERADE"]
        TERM --> TAP --> FWD
    end

    L2G -->|"Ethernetフレーム"| NIC
    L3H -->|"WSS"| TERM
    FWD --> NET["インターネット"]
```

ポイントは**層の数ではなく、層の切り方**にある。

- ①のLinuxは、自分の下にあるものが「Ethernetケーブル」だと思っている。
  実体が②のWebSocketであることを知る手段が無いし、知る必要も無い。
  **層とは物理的な事実ではなく、約束事である**ことがそのまま見える
- ②はL1の役を演じているだけなので、我々が実装するのは
  「フレームを渡す・受け取る」という物理層の契約だけになる。
  L2以上を1行も書かないのは、浅いところに降りたご褒美である

#### 「下」と「難しい」は別の軸である

L1は最下層なので「一番深い」と感じるのが自然だが、
**位置の深さと難度は別の話**である。

現実のL1が難しいのは**物理**があるからで、銅線の減衰、光の分散、変調方式、
信号品質 — ここは確かに深い。だがL1が**上位層に対して負っている契約**は
「フレームを1枚渡す。ときどき落ちる」だけで、拍子抜けするほど単純である。

N3では「ケーブル」がWebSocketなので、**その物理が丸ごと存在しない**。
減衰も変調も無く、残るのは契約だけになる。だから実装が数十行に落ちる。
L1の役を引き受けたのに軽いのは、深さから逃げたのではなく、
**深い部分 (物理) を持たない媒体を選んだ**からである。

難度は層番号に比例しない。

| 層 | 上位への契約 | 実装の重さ |
|---|---|---|
| L1 | フレームを渡す。落ちてよい | 軽い (物理が無ければ) |
| L2 | 宛先MACへ届ける。ARPで解決 | 軽い |
| L3 | 経路を選ぶ。断片化を扱う | 中 |
| L4 (TCP) | 順序・再送・輻輳制御 | **重い** |

#### ただし、この浅さが TCP over TCP を生む

図をよく見ると、**①のTCPが②のTCPの中に入れ子になっている**。
これは層の契約違反である。上位のTCPは「下の層は不確実で、落ちるときは落ちる」
という前提で輻輳制御を設計している。ところが②のTCPは再送して届けてしまう。

結果、②が再送で粘っている間に①も独立にタイムアウトして再送を始め、
再送が再送を呼ぶ。VPNがUDPを好む理由がここにある。

注目すべきは、**どの層も単体では正しく動いている**ことである。
①のTCPも②のTCPも仕様どおりに振る舞っている。それでも壊れる。
壊しているのは層ではなく、**下の層が契約以上に頑張ったこと** —
「落ちてよい」はずの媒体が、落とさず必ず届けてしまったこと — である。

したがって、この設計で深いのは層の中ではなく**層と層の境界**にある。
N3はその境界を実測できる装置になっている。
