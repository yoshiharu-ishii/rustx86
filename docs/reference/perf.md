...
| **F1d** | **テンプレートJIT (AArch64手書き+dynasmrt、Craneliftなし)** | 185+ MIPS | 🔬**進行中** | a骨格→bロード→cストア/スタック→d TLBインライン→e 定常WL定規→f 8bit語彙+損益再審 (PR #174〜#181)。**指紋2コース (jboot/jcmd) でon/offビット同一**。F1d-f交互3周: ブート off≈on (**互角、3周目は勝ち**) / gcc窓 off 67.5 vs on-main 47 (-30%) vs on-f1d-f 57 (-15%) = **赤字半減**。窓カバレッジ58%→降格入りで51%・平均7.8命令/入場。授業料: JSLOTS 256Kは衝突再焼き359k回で-13 MIPS (2Mへ)。残る赤字は生成コードの実行密度 (毎opロード/ストア+cc6ストア+ヘルパ呼び) — 次はレジスタ/cc常駐化の設計判断。Stage 2でブロック粒度 (案B) とC14解禁。Stage 3 (wasm) は前倒し済み (#180) |
