...
素子は [`chip::dp8390`](../../core/src/dev/chip/dp8390.rs) 1つで、
[`card::ne2000`](../../core/src/dev/card/ne2000.rs) と
[`card::rtl8029`](../../core/src/dev/card/rtl8029.rs) は基板の都合
(PROMの並べ方・設定空間の名乗り) を着せるだけである。
...
そこで**カードの前に線を置く** ([`rx_queue`](../../core/src/dev/chip/dp8390.rs)):
...
## 本測定 — 遅延の内訳 (Tier 5d、2026-08-15)

スループットの次は往復時間である。定規は
[`tools/webtest/netlat.mjs`](../../tools/webtest/netlat.mjs)。**宛先は 10.0.2.2**
(SLiRPのゲートウェイ) — wsslirpのnetstackが自分で答えるので**外の経路が混ざらない**。
スループットで「測っていたのはスウェーデンだった」と踏んだ罠の対策である。

### 16bit と 32bit

| ゲスト | 刻み | RTT min/avg/max |
|---|---|---|
| FreeDOS + mTCP | 5万命令 | **0.85 / 1.27 / 1.70 ms** |
| Linux | 30万命令 | 13.9 / 20.2 / 37.1 ms |
| Linux | 10万命令 | 4.8 / 9.2 / 17.0 ms |

**16bitが速いのは偶然ではない** — ハーネスの刻みが6〜20倍細かいだけである。
ただし**16bitのハーネスには時計の轡が無い**ので、その 0.85ms は*仮想時間*での値で、
実時間ではおよそ倍になる。**轡の有無で数字の意味が変わる**ことは、比べる前に確かめること。

### 遅延は「刻み」と「寝」でできている

32bit側で刻みを振ると、RTTは刻みに連れて動くが**比例しない** — 刻みを細かくするほど
「RTT ÷ スライスの実時間」が増える。つまり刻みに依らない**定数項**が居る:

| 刻み | スライスの実時間 | RTT avg | avg ÷ 刻み |
|---|---|---|---|
| 1000万 | 110.1 ms | 126.2 ms | 1.15 |
| 300万 | 40.7 ms | 44.2 ms | 1.09 |
| 100万 | 16.8 ms | 28.2 ms | 1.68 |
| 30万 | 5.4 ms | 20.2 ms | 3.73 |
| 10万 | 1.8 ms | 9.2 ms | 5.11 |

正体は**アイドル時の寝**だった。pingの合間にゲストはHLTで寝るので、早送りが働き、
ランナーは実時間との釣り合いを取るために**最大50ms寝る**。その間に応答が届いても、
**寝終わるまでゲストは動かない**。

`WAKE=1` (寝ている間に届いたら起こす) で測り直すと:

| 刻み | 素の RTT avg | 早起き有り | 差 |
|---|---|---|---|
| 30万 | 20.2 ms | **13.7 ms** | **−32%** |
| 10万 | 9.2 ms | 7.8 ms | −14% |

**刻みが粗いほど効く** (寝る時間が長いから)。ブラウザのワーカーは忙しいとき
~8ms (≒30万命令) に落ち着くので、実働点はちょうど効きの大きい側にある。
そこで [`web/linux-worker.js`](../../web/linux-worker.js) に早起きを入れた —
`net-rx` が届いたら寝ているタイマを切って `loop()` を回す。

**時計の轡は壊れない。** 早送りで進めた分は既に仮想時間に計上済みなので、
早く起きても借りは増えない — 寝るのを短く切り上げるだけである。

### 測って棄却した案: 送信の早出し

受信側 (早起き) の対になる手として、**ゲストがフレームを吐いたら `run_slice` を
途中で戻す**案を実装して測った。行きの待ち (平均0.5スライス) が消えるはずで、
実際に消えた — が、**スループットが割に合わなかった**:

| | RTT (刻み30万) | wget 1MB (同一セッション交互A/B) |
|---|---|---|
| 早起きのみ | 13.7 ms | **1.05 / 1.09 s** |
| + 送信の早出し | 11.9 ms (**−13%**) | 1.32 / 1.64 s (**30〜50%悪化**) |

実行命令数は同じ (21〜22M)・取りこぼしも0%なので、**エミュレータの仕事は増えて
いない**。伸びたのはスライスが細切れになった分の**外側の税** (JS往復と轡の寝) である。
遅延13%のために帯域を3〜5割払う取引にはならない。

**前提が変われば再訪する**: ワーカーの刻みが恒常的に10万命令級まで細かくなったら
(そこでは早出しの取り分が −33% と大きい)、あるいはスライス外の税が減ったら。
実装は `Machine::net_has_tx()` (真偽値1つ) と `run_slice` の break 1行で戻せる。

なお**会計の直しは残した** — `run_slice` は「実際に進んだTSC」を返し、走らせる側は
それで仮想時間を数える。早く戻る仕組みが無くても、これは元々そうあるべき形である
(pitfalls 7「予算に含まれるものを外から足すな」の対)。

### 残り

刻みと寝を除いた残差 (30万命令で ~3ms) は、ゲストのスタックとNICの割り込み処理。
ここを分解するなら `bootprof` と同じ ip サンプリングを ping 中に当てる。
