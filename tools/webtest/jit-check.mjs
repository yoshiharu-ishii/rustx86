...
async function boot(useJit) {
  ...
  const jinstrs = useJit ? emu.jit_instrs() : 0;
  return { banner, serial, instrs, installed, baked, entries, jinstrs };
}
...
console.log(`[diag] baked=${on.baked} entries=${on.entries} instrs=${on.jinstrs}`);
