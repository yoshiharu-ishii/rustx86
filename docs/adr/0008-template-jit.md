...
## 追記 (2026-08-12): F1c関門プローブの結果 — 通過、ただし背景焼きが設計要件

tools/jit-probe-native (Cranelift 0.116) で固定費を実測した (perf-log.md 同日):
呼び出し~3ns・uop単価0.3〜0.8ns (インタプリタの20〜50倍の頭上) で**回収可能**。
ただし焼きは53〜276µs/ブロックとwasm手組みの~10倍で、熱ブロック全量 (~31k) の
前払いは~2.2秒 — **焼きは背景スレッドで行い、できたブロックから差し替える**
のがF1cの設計要件になる (ネイティブの特権。wasmでは選べなかった)。
opt_level none/speed は焼き・実行ともほぼ差がなく、speedでよい。
本実装の設計 (差し替えの決定性・メモリuopヘルパの形) は着手時に別ADRで。
