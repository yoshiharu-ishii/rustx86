...
fn main() {
    ...
    m.boot_bzimage_with_initrd(&data, "console=ttyS0", initrd.as_deref())
        .unwrap();
    let mut timer_at_10b = 0u32;
    for i in 0u64..12_000_000_000u64 {
        if m.trap.is_some() {
            println!("trap @{i}: {:?}", m.trap);
            break;
        }
        m.step();
        if i == 10_000_000_000 {
    ...
    let bytes: Vec<u8> = (0..12)
        .map(|k| m.read_phys8(m.translate(m.cpu.lin(cpu::CS, ip + k))))
        .collect();
    let asm = rustx86_disasm::one(&bytes, 32, ip as u64);
    println!(
        "EIP={ip:08x} {asm} IF={} halted={}",
        m.cpu.flag(cpu::IF),
        m.halted
    );
    ...
    println!(
        "pit c0: reload={} count={} mode={} running={}",
        m.devices.pit.counters[0].reload,
        m.devices.pit.counters[0].count,
        m.devices.pit.counters[0].mode,
    ...
    println!(
        "int_recent: {:?}",
        m.int_recent.iter().rev().take(6).collect::<Vec<_>>()
    );
}
