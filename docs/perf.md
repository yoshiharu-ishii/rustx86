...
### ホスト横断の実測 (2026-08-11、同じイメージ・同じ命令数)

| ホスト | ネイティブ bzImage (970M) | ネイティブ vmlinux (580M) | wasm bzImage (1000M) |
|---|---|---|---|
| M1 MacBook Air (2020) | 10.7s / **90.7 MIPS** | 7.3s / 79.5 | 16.2s / 61.7 |
| Ivy Bridge素のWindows (2012) | 23.9s / 40.6 | 15.2s / 38.2 | 未測定 (node未導入) |
| 同マシンのVirtualBox VM (Ubuntu) | 26.1s / 37.2 | 16.0s / 36.3 | 36.0s / 27.8 |

読み方:

- **仮想化の税はわずか5〜9%** (同一ハードのネイティブ vs VM)。エミュレータは純粋な
  ユーザー空間演算でVM exitをほぼ起こさないため。M1との2.4倍差の正体は
  **CPU素の世代差** (2020 vs 2012) がほぼ全て
- **wasm/ネイティブ比は7割前後**で、ISA (arm64/x86_64) によらず安定
- **命令数は全5環境でビット同一** (970M/580M/1000M) — 決定性がOS・ISA横断で成立。
  Tier 8 (ホスト混在のスナップショットfork) への布石
- Windowsビルドは MSVC で素通り (14.9s) — **Tier 7の主要リスク (Windows 10ビルド) は消えた**
...
| # | 案 | 期待 | 状態 | メモ |
|---|---|---|---|---|
| F1 | テンプレートJIT (wasm/ネイティブ生成) | 5〜20倍 | **🎯本命昇格** | B1-B5の結果が出た (2026-08-11): インタプリタ構造の中の再配置は D2/C5/B5/TLB配列化 が全滅で、**構造の上限 (~40サイクル/命令) に到達**。ここから先は実行そのものを変える = JITだけ。決定性を壊さない設計が前提 |
