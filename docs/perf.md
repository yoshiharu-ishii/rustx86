...
順序:   C1 lazy flags (10〜25%、cosim全緑+命令数不変が関門) → C3/D1/D2の小物
        → 届かなければ切り札B5トレース化 (JITは温存継続)
現在地: 素で66 MIPS、PGOビルドなら **~74 MIPS**。100到達で店じまいし、
        マイルストーン (README参照): ANSI端末DOOM → Tier 6a → VGA DOOM へ
```
