...
///
/// **世代の前提はF1c時代から変わった (ADR-0020)**: 素の線形ストアも世代を
/// 進めるようになった。よって**ストア語彙 (StoreMR等) を生成コードで有効に
/// する前に**、「ヘルパが自ページの世代を進めたら実行済みn+1で脱出」の受けを
/// 足すこと (F1d-aはレジスタ語彙のみなので未着手でよい — ブロック内で
/// 世代は動かない)。
///
...
/// 遅延フラグの cc_op 特殊値 (cpu/mod.rs の私有定数の公開鏡 —
/// INC/DEC はCF不変の別口。生成器がset_cc_incdec相当を書くのに要る)
pub const CC_INC: u8 = super::super::CC_INC;
pub const CC_DEC: u8 = super::super::CC_DEC;
/// 焼けたブロックの実行フック (F1d-a)。
///
/// coreは生成器もランタイムも知らない。F1cの「Entryにスロットを畳む」受け口は
/// 受け口の税 (+8B/Entry、削除で-22%) だったので、F1dでは**ブロック頭でだけ**
/// ランタイムに問い合わせる形に変えた — 毎命令のホットループには1バイトも
/// 足さない。呼ばれるのはチェーン入口 (step_cachedの先頭)。
///
/// dynではなくfnポインタなのは coreの無依存を保つため。生成コードは
/// Machineのフィールド実番地 (layout) を直接読み書きする — 入口が不透明な
/// 関数ポインタなのでコンパイラは呼び出しをまたいだ別名最適化をできない。
#[derive(Clone, Copy)]
pub struct JitHook {
    /// ブロック頭 `pa` に焼けたブロックがあり、世代が `gen` と一致し、
    /// 全長が `budget` 以下なら実行して**実行命令数**を返す。0 = 入らなかった
    /// (未焼き・世代落ち・予算不足・語彙外)。F1d-aは全量実行のみ —
    /// 途中脱出 (フォールト・予算) は語彙をメモリ形へ広げるときに再導入する。
    /// 実行後の ip は生成コードがブロック出口で1回書いている。
    /// `ctx` はランタイム自身の生ポインタ — TLS参照 (macOSはtlv呼び1本
    /// ≈5-10命令/入場) を入場路から消すための持ち込み品
    pub try_enter: fn(ctx: usize, pa: u32, gen: u32, budget: u32) -> u32,
    pub ctx: usize,
}

/// ブロックのページ世代 (焼いた時点の値を控えて、実行前に照合する)。
/// F1d-aの語彙 (レジスタ形のみ) はブロック内で世代を動かせないので、
/// 頭での照合がインタプリタの毎命令照合と同じ強さになる。
/// **ストア語彙を足す日には JitOp のdoc (n+1脱出の受け) を先に読むこと**
