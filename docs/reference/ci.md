...
flowchart LR
    ...
    subgraph L4["4 機能層 (mainのみ)"]
        S[スナップショット<br>往復ビット不変] & P[貼り付け<br>欠けずに届く] & N[ネットワーク<br>DHCP→ICMP→HTTP]
    end
...
| ネットワーク | NICがゲストから見えない・DHCPが通らない・ICMPが往復しない・TCPが流れない・**ゲストの時計が実時間から外れる** | 起きた (フロッピー機がPCI付きになって16bitからNICが消えた。時計が外れてpingが洪水になった) |

### ネットワークの関門は外に出ない

ping先は 10.0.2.2 (SLiRPのゲートウェイ = wsslirpのnetstackが自分で答える)、
HTTPはジョブが立てた自分のサーバ。**相手のサイトの都合で赤くならず、ICMPの
権限も要らず、誰かの本番にも当たらない。** ゲストが通る道 (NE2000/RTL8029 →
WebSocket → netstack) は実物と同じなので、回帰の値打ちは落ちない。

TLS (https) だけはここで見られない — 閉じた世界に信頼できる証明書が無い。
MMX・SSE2の語彙・RTCの実時刻・CA束の同梱がまとめて要る検査なので、
実インターネット向けの手動E2E (`tools/webtest/netlinux.mjs` を既定の宛先で
走らせる) が受け持つ。**自動で見張れていない穴として、ここに明記しておく。**
...
`*-hdd.img.gz` を拾えなかった。HEAD から消しても旧 SHA で取れるので、履歴の書き換え
(filter-repo + main の保護を秒単位で外して force push) で消す羽目になった。
...
- **サーバ側 (push ruleset の max_file_size)** は Organization 配下のリポジトリ限定で、
  ...
  Organization に移したら付ける

像 (ディスクイメージ・initramfs・ISO・スナップショット) は rustx86-images と
`tools/images/sh/fetch-images.sh` が受け持ち、web/ には置くが commit しない (`/web/*.img.gz` 等)。
...
- イメージは GPL 配布物なのでリポジトリに置かず、入手経路は
  `tools/images/sh/fetch-images.sh` ただ一つ (CIはActionsキャッシュに控えるだけ)
