...
| 3 | 32bit化 — モード・ページング + **速さと道具の足回り** | 拡張中 (高速化=docs/perf.md) |
...
- [ ] **3f: 32bitの道具** — ワーカー越しデバッガ (Linuxマシンは Worker の中で
      回るので今のデバッガから覗けない)。スナップショットのファイル書き出しは
      JSON (gzip+base64で数割膨れる) をやめ、**バイナリ形式 (レジスタ+メモリのzip)**
      で作り直す — UIのボタン群は3OSで統一済み
...
```bash
python3 web/serve.py 8001
# http://localhost:8001/ を開き、左メニューの「実行速度ベンチ」を選ぶ
#   計測だけの静かな部屋が要るなら http://localhost:8001/bench.html
```
...
ベンチは `document.hidden` を見て汚染された回に ⚠ を付け、結果を無効として扱う。
この交絡は追試の途中で実際に踏んだ。
