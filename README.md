...
**結果: 70.0 / 104.9 / 106.0 MIPS** (Chrome / Apple Silicon)。

```
ネイティブ 最良 105.8 MIPS
WASM       最良 106.0 MIPS   ← ほぼ同じ
```

**ウォームアップ後のWASMはネイティブとほぼ同速**だった。ブラウザで動かすこと自体は
速度上の障害にならない、という見通しが立つ。

ただし測り方に2つ罠がある。どちらもページに注記してある。

- **1回目は最良の66%しか出ない。** JITのティアアップ (ベースラインコンパイラで
  走り出し、実行中に最適化コンパイラへ切り替わる) と、wasmリニアメモリの
  初回確保がそこに乗る。1回目を基準にすると存在しない性能差を見ることになる
- **1回が短すぎる測定は低く出る。** ウォームアップの占める割合が大きくなるため、
  同じワークロードでも5000万命令 (約0.6秒) では3.7億命令 (約3.5秒) より
  2割ほど低い値になる。数秒かかる長さで測ること
