...
#### 踏み台はまずローカルに置き、必要になってからEC2へ出す

「ページを配る `http://localhost`」と「踏み台」は**別のもの**である。
前者は静的ファイルを返すだけで、フレームの中継には関与しない。
だが踏み台自体をローカルに置くことはでき、**最初はそのほうがよい**。

| | フェーズ1: ローカル踏み台 | フェーズ2: EC2 t3 |
|---|---|---|
| 接続先 | `ws://127.0.0.1:9000` | `ws://<GIP>:9000` |
| 開放プロキシのリスク | **構成上ゼロ** (外から到達不能) | SGとシークレットで防ぐ |
| 費用 | なし | インスタンス + 転送量 |
| 経路 | 二重NAT (VM → Mac → 家庭用ルーター) | 素直な経路 |
| 自宅LAN外から | 不可 | 可 |

ローカルで足りなくなるのは、**MTUやレイテンシの実測値が
家庭用ルーターの都合で濁るとき**、および自宅LANの外から使いたいときである。
そこまでは費用ゼロで進められる。

ただし **macOSにはTAP (L2) デバイスが無い**。Appleの `utun` はL3の
ポイントツーポイントでEthernetフレームを扱えず、kext版のtuntaposxは
Apple Siliconと現行のSIPで動かない。したがってローカル踏み台も
**Linux VM / コンテナの中**に置く。

```
docker run --rm -it --cap-add=NET_ADMIN --device /dev/net/tun -p 9000:9000 relay
```

Docker Desktop の実体はLinux VMなので、コンテナ内で `ip tuntap` と
`iptables MASQUERADE` が本物として動く。ただしホスト側のネットワークが
ユーザーランド実装のため、**ゲストのICMPが外まで抜けるかは要検証**。
`ping` が到達目標に含まれる以上ここは確認する。抜けない場合は
Lima / Colima / UTM で素のLinux VMを立てる。
...
### 8. N3の設定は「起動前にGIPを入れる」だけにする

N3はエミュレータ側に翻訳処理が無いため、**外から与える情報は踏み台のアドレスだけ**で足りる。
ブート前に次を選ばせる。

| 設定 | 効果 |
|---|---|
| ネットワーク OFF (既定) | **仮想NICをゲストに見せない** |
| ネットワーク ON | 踏み台のアドレス (と共有シークレット) を入力。virtio-netを生やす |

アドレスはフェーズ1なら `127.0.0.1:9000`、フェーズ2ならEC2のGIPになる。
**エミュレータから見れば区別が無い**ので、切り替えは入力欄の値だけで済む。

OFFのときNICを見せないのは、NICだけ在って誰も応答しないと
**Linuxが起動時のDHCPで数十秒タイムアウトを待つ**ためである。
装置ごと消せば、ゲストは最初からネットワークが無い構成として起動する。

入力値は localStorage に保存する。EC2は停止/起動でGIPが変わるので、
Elastic IPを割り当てておくと入力し直さずに済む。

#### `ws://` と `wss://` の分岐は、公開しない決定から自動的に決まる

- ページが `https://` の場合、そこから `ws://` は**混在コンテンツとしてブロック**される。
  `wss://` が必須になるが、生IPにはLet's Encryptが証明書を発行しない
- ページが `http://localhost` の場合、`ws://<GIP>:port` が**そのまま通る**。証明書は不要

N3は決定5により公開しない。開発はローカル配信で行うため、
**証明書の問題は発生しない**。「GIPを入れるだけ」が文字通り成立する。

将来ローカル以外から使いたくなった場合は、`<GIP>.sslip.io` のような
IPを埋め込んだDNS名でLE証明書を取る手がある (要検証)。
