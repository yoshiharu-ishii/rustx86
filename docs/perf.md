...
相殺 — ADR-0008の「測って決める点」の答えが出た。**次の梃子はTLBヒット経路の
インライン生成** (ヘルパを呼ばずに生成コードでTLB照合→直読み)。
...
| # | 案 | 期待 | 状態 | メモ |
|---|---|---|---|---|
| F1 | テンプレートJIT (wasm/ネイティブ生成) | 5〜20倍 | 🔬F1a骨格済 | B1-B5全滅で本命昇格 (2026-08-11)。**F1a骨格が動いた** (2026-08-12): ホット検出→wasm生成器 (レジスタ間mov/ALU/jcc/lea)→instantiate→実行のパイプが通り、**JIT on/offで命令数・シリアル出力ビット同一** (jit-check.mjs、CIの決定性ゲート)。Linuxブートで339ブロック発火。JIT無効時の退路は無コスト。設計はADR-0008 (案A: 毎命令の受付点維持)。**F1b-1 (ロードのテンプレート化、フォールト脱出モデル) 済** (2026-08-11): ブロック344→668、素朴版 (ヘルパ経由) は**ワッシュ**。次の梃子はTLBヒット経路のインライン生成。**上の実測を参照** |
