...
| **C15** | **core第4ラウンド** ([ADR-0027](../adr/0027-core-round4.md)) — フォールバック税と具現化税 | 5-10% (interp側) | 🔬**進行中** | PR1 **StrRep+負キャッシュ採用**: REPを語彙へ (勘定=REP全体1命令のまま、指紋は値も不変)。gcc窓 off +3-4% (80→83 MIPS)・on +3% (86→89) 全勝、ブートon -0.1〜0.2s。次: PR2 部分materialize → PR3 0x66語彙 (センサス後)。台帳: [core-roadmap](core-roadmap.md) |
