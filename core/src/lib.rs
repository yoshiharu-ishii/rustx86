...
/// HotState (D1、2026-08-13): 命令境界と変換経路が**毎命令**触るスカラと
/// 配列ヘッダを先頭の1〜2キャッシュラインに束ねる。従来のrustc任せの
/// レイアウトでは line 4/21/28/31/33/34 の7〜8ラインに散っていた (実測)。
/// repr(C)は宣言順を守らせるため — スナップショットはフィールド単位の
/// 手書きなのでレイアウト非依存 (snapshot.rs)。実測レイアウト:
/// line 0 = tick〜trap頭 / line 1 = tlb・mem・dcacheヘッダ / line 4〜 = cpu
#[repr(C)]
pub struct Machine {
    // ---- 熱い塊: ここから2ライン以内に毎命令の検査対象を全部置く ----
    /// 装置を進めるまでの残り命令数。
    ///
