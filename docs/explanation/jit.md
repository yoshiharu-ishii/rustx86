...
- **現在地**: **F1d 税なしテンプレートJIT** ([ADR-0022](../adr/0022-cpu-round3-before-gfx.md))。
  F1d-a骨格が完成 — dynasmrt (AArch64手書き、追加数十kB) でビット同一一発、
  受け口はチェーン入口のみ (Entryに畳んだF1cの+8B税を再導入しない)。
  次はF1d-b: メモリ語彙 (世代bump脱出の受けが前提)・タイル張り・Jcc両側焼き
- **その先**: ブロック粒度実行 (案B再訪 — icount式切り直しで命令数決定性を
  保ったままdead-flags等を解禁) → wasmバックエンド解凍でブラウザへ還流
