...
## 起動済み状態イメージの配布 (v86/Firecracker式) — 💤保留 (ユーザー裁定)

**ブートの970M命令を丸ごと消す、この台帳で唯一の「桁」級。** ただし
2026-08-20ユーザー裁定で**見送り**: 「根本の解決じゃない」— ブートを
飛ばすのは荷物の隠し方であって、エミュレータ自体を速くする設計ではない
(「速さは設計で取る」の家風)。**再訪条件: ネイティブアプリを作るとき**
(そこでは起動レイテンシが製品仕様になるため)。
...
即日枠は2026-08-20に裁定済み (シリアル=済み・-O3/mimalloc=ワッシュ)。
状態イメージ配布は保留 (再訪条件=ネイティブアプリ化)。イメージ側 (boot
args/linux-virt) は着手時に「新コースの絶対時間」で裁く。速度の正面戦は
jit-roadmap3の常駐化バンドルへ。
