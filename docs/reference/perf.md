...
| F1c | Cranelift ネイティブJIT | 185 MIPS超 | ❄️凍結 | 2026-08-13 [ADR-0013](../adr/0013-f1c-freeze.md)。関門〜予算退出まで完成、相対-9〜13%・カバレッジ46%だが**リンク税+25%で絶対は素のcoreに+19%負け**。実装は `f1c-final` タグ、実験は `exp/f1c-*`。復帰条件はADR (定常WL/連鎖を削る深さ/税の消滅) |
...
## ホットパス付帯処理の一掃 (2026-08-13の作戦 — 15.1s→10.9s、-28%)
