...
最後の行は Tier 4 でもう一度出てくる。同じチップの同じレジスタが、
32bit化の障害物として顔を出す。
...
| 追加するもの | BIOSへの影響 |
|---|---|
| Tier 3 NE2000 | **ゼロ。** イーサネットにBIOSサービスは存在しない (PXE/UNDIは後付けの別物)。OSは自前ドライバで叩く |
| Tier 4 32bit化 | **ゼロ**。むしろ使われなくなる |
| Tier 5 メモリ増設 + Linux | **説明書だけ増える。** カーネルにRAM量を教える者が要る。bzImage直接ロードでは `boot_params` に e820 を自分で書く — サービスではなく説明書の仕事 |
