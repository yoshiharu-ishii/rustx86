...
fn main() {
    ...
    while i < 30_000_000_000 {
        ...
        if m.devices.uart.tx.len() > printed {
            let out: String = m.devices.uart.tx[printed..]
                .iter()
                .map(|&b| b as char)
                .collect();
            print!("{out}");
    ...
    eprintln!(
        "[done: {i} instrs, serial {} bytes, EIP={:08x}]",
        m.devices.uart.tx.len(),
        m.cpu.ip
    );
    // VGAテキストも覗く
    for row in 0..25 {
