...
///
/// **64→256 (2026-08-18、ADR-0025)**: tick粒度は実行エンジンの「往復」の
/// 天井でもある (チェーン/JITの予算 = min(鎖残り, tick残り))。256で
/// gcc課程が全体-20%・JITがインタプリタを窓でも上回る。ゲスト可視の
/// タイミング変更なので、指紋・golden・RTTの基準は張り替え済み
/// (netlatの実測はperf-log 2026-08-18)
...
/// **64命令あたり1クロック**が校正の原点。tick粒度を変えるときはこの比を
/// 保つこと — 粒度だけ4倍にしてクロックを1のままにすると、ゲスト時計が
/// 命令比で1/4に遅くなり、benchカーネルがデッドハルトした (2026-08-18に実際に
/// なった)。粒度が変えるのは「配送のまとめ幅」だけで、時間の換算は不変が契約
pub const PIT_CLOCKS_PER_TICK: u32 = INSTRUCTIONS_PER_TICK / 64;
