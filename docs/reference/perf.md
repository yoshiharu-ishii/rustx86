...
| **F1d** | **テンプレートJIT (AArch64手書き+dynasmrt、Craneliftなし)** | 185+ MIPS | 🔬**進行中** | a骨格→bロード→cストア/スタック→d TLBインライン→e 定常WL定規+正しさ穴の修正 (PR #174〜#178マージ済み)。**指紋2コース (jboot/jcmd) でon/offビット同一**。冷間: ブート0勝5敗+6% / gcc窓 off 54.6 vs on 48.6 MIPS = **まだ赤字**。gcc窓カバレッジ49.7%だが平均ブロック1.8命令 (cc1の8bit演算が語彙外)。次 (F1d-f): 8bit語彙・入場税。Stage 2でブロック粒度 (案B) とC14解禁、Stage 3でwasm解凍 |
