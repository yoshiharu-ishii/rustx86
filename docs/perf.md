...
| # | 案 | 期待 | 状態 | メモ |
|---|---|---|---|---|
| F1 | テンプレートJIT (wasm/ネイティブ生成) | 5〜20倍 | 🔬F1a骨格済 | B1-B5全滅で本命昇格 (2026-08-11)。**F1a骨格が動いた** (2026-08-12): ホット検出→wasm生成器 (レジスタ間mov/ALU/jcc/lea)→instantiate→実行のパイプが通り、**JIT on/offで命令数・シリアル出力ビット同一** (jit-check.mjs、CIの決定性ゲート)。Linuxブートで339ブロック発火。ただし**ブート速度は同着** — レジスタ形だけではブートを支配するメモリ操作をカバーしないため (想定どおり)。速度はF1b (メモリuopのテンプレート化) の仕事。JIT無効時の退路は無コスト (交互A/Bで現mainと同着)。設計はADR-0008 (案A: 毎命令の受付点維持) |
