...
| 経路 | シェル到達 | 実効MIPS |
|---|---|---|
| ネイティブ vmlinux (**370M** — ブートダイエット後) | **5.7s** (熱あり) | ~65 |
| ネイティブ bzImageフル (~760M) | — | — |
| wasm headless vmlinux (~370M) | **8.8s** | ~45 |
| wasm headless bzImageフル | 17.0s | ~47 |
| ブラウザ vmlinux (推定) | **~9-10s** | — |
| スナップショット復元 | ~1s | 起動しない、が最速 |

**ブートダイエット (2026-08-12)**: 命令数そのものを 580M→**370M (-36%)** に削った
(initramfs非圧縮化 -50M + SETUP_RNG_SEED -160M)。内訳と手法は
[perf-log.md](perf-log.md) 2026-08-12ブート解剖の項。残る大物は
ftrace/kallsyms初期化 ~145M = E5 (自前カーネルconfig) の獲物。

- 出発点は13 MIPS — **約6倍**。1命令~40サイクルの壁は実測で「構造」と確定
