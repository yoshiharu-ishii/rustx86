...
impl Counter {
    ...
    fn advance(&mut self, n: u32) -> u32 {
        ...
        let rest = n - cur;
        // ワンショットの面々: 0に達したら1回だけ鳴って止まる。
        //   0 = 割り込み (端子カウント)   1 = 再トリガ可能ワンショット
        //   4 = ソフトウェアストローブ    5 = ハードウェアストローブ
        // **Linuxのhresティックは mode 4 を使う** (i8253.c の set_oneshot)。
        // ここを周期扱いにすると「腕を上げた覚えのない割り込み」が0.94msごとに
        // 鳴り続け、jiffiesが実時間の数十倍で流れる — sleep 3 が一瞬で明け、
        // ping 1.1.1.1 が本物のインターネットへ毎秒数百発の洪水になった (実話)
