...
| **S1** | **エンジン外の鉱脈 (第5ラウンド調査)** | 起動は「桁」級 (状態イメージ) | 📋台帳 | core/JIT以外の全部 — 本命は**起動済み状態イメージ配布** (v86/Firecracker式、ブート970Mを丸ごと消す。土台のsave_state/from_snapshotは実装済み)。ほか boot args・linux-virt・シリアルバッチ・遅延ブロック取得・wasm-opt -O3・mimalloc等。イメージ側は**コース変更=指紋張り替え**なのでA/Bと帳簿を分ける。台帳: [system-roadmap](system-roadmap.md) |
