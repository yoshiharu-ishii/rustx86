...
## Fシリーズのロードマップ (2026-08-11 引き直し — F1b-3の実測を反映)

F1b-1〜3の実測で地図の解像度が変わった: **語彙は既に81%・カバレッジ機構は完成
(18.1%)・残る本丸は単価** (メモリopのヘルパはインタプリタと同じ仕事+呼び出し税で、
ディスパッチ除去だけでは1命令あたりの収支が並ぶ)。ここから先はこう歩く:

```mermaid
flowchart TB
    DONE["済: F1a骨格 + F1b-1/2/3<br>3ISAビット同一・脱出モデル・<br>カバレッジ機構 (タイル張り/走路焼き/永続台帳)<br><b>現在: カバレッジ18.1%、収支は微赤字</b>"]
    F1B4["<b>F1b-4 単価: TLBヒット経路のインライン生成</b><br>ロード/ストアの「TLB照合→直読み書き」を生成コードに焼く<br>ヘルパはTLBミス時だけ (意味論は原本のまま)<br><b>Fシリーズ最大の関門 — 初の明確な黒字を狙う</b>"]
    F1B5["F1b-5 語彙と密度<br>shift r (5.2%) / movzx (3.9%) / test8 / 8bit系<br>+ cond/cf のインライン化<br>語彙81→90%+、平均ブロック長4.4→7+"]
    F1B6["F1b-6 ブラウザ配線と出口測定<br>ワーカーへ setupJit/pumpJit (~10行)<br>ブラウザ実測で黒字を確認"]
    EXIT{"出口判定<br>(ADR-0007の流儀)"}
    TIER["一旦停止してTierを進める<br>(GUI/Tier6・microVM/Tier8)<br>台帳に凍結理由を記録"]
    F1C["F1c: Cranelift ネイティブバックエンド (Tier 7と同時期)<br>フロントエンド (脱出モデル・語彙・カバレッジ機構) は全部共有<br>F3 (ホストフラグ流用) はこの中でだけ意味を持つ"]

    DONE --> F1B4
    F1B4 -->|"黒字"| F1B5 --> F1B6 --> EXIT
    F1B4 -->|"それでもワッシュ"| TIER
    EXIT -->|"wasm headless 10s切り<br>(≒60 MIPS) 到達 or 頭打ち"| TIER
    TIER -.->|"Tier 7で再開"| F1C
```

- **見積り**: wasm編の決着 (黒字化 or 撤退判断) まで**あと2〜4セッション**。
  F1b-4が1〜2、F1b-5が0.5〜1、F1b-6が0.5
- **出口条件**: wasm headless (vmlinux) で**10秒切り (≒60 MIPS)**、または
  **2増分連続でワッシュなら一旦止めて**Tier進行へ戻る (切り札は温存 — ADR-0007の
  「Tierの節目ごとに最適化判断」と同じ流儀)。だらだら続けない
- **概算の裏付け**: 単価1/3 (インライン化) × カバレッジ50% (F1b-5で密度が上がると
  entriesあたりの命令数が伸びる) で -35%。headless 14s → 9s台が射程
