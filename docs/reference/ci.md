...
| 音 (PCスピーカー) | ゲストのOUT命令 → PIT → 周波数の列。単体では「鳴った気になる」までしか見えない | AIR.COM を手で鳴らして確かめている。周波数列の期待値と突き合わせれば自動化できる |
| 画面の操作そのもの (右クリック・テーマ・ドラッグ) | DOMが要る。判断は decide.js で単体化したが、**配線の間違い**は残る (実際、SVGの `hidden` に代入して見た目が変わらない事故を踏んだ) | 実ブラウザでの手動確認。Playwrightを入れるかは未決 |
