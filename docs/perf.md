...
**実機Chrome確認 (2026-08-12 ユーザー実測): bzImageフル 43s→約23s、
vmlinux 21s→15s** — headless 14.0〜14.5sに対し残るタブ税は**~7%**。
×1.5の税はこの1行の修正でほぼ消えた。ブラウザの
...
| C7 | メモリuopのdTLBインラインキャッシュ (uopに前回の(ページ,物理)を控える) | 3〜5% | 💤 | 台帳外の棚卸しで追加 (2026-08-12)。translate_for 5%の内側狙い。B5の教訓 (ホットループへのチェック追加は逆効果) と同型のリスクあり。**実測済み (2026-08-12): ワッシュ** — CPL0限定・read/write別の最終結果1段キャッシュ (正しさはtlb_flush世代で閉じる設計) を組んだが、native 6周 (8.4 vs 8.5s)・wasm 4周 (13.6 vs 13.7s) ともワッシュ。B5の教訓の再演: TLBヒット経路はOoOが既に隠している。タグ [exp/c7-dtlb](https://github.com/yoshiharu-ishii/rustx86/releases/tag/exp%2Fc7-dtlb) |
...
- **panic=abort** (2026-08-12): unwind経路の除去。交互A/B 8周で**ワッシュ**
  (中央値 9.4 vs 9.45s)。LTO+codegen-units=1 (A1) が既にランディングパッドの
  大半を最適化済みと推測。panic意味論を変えてまで取る差が無い — 不採用
