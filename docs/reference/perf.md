...
| A3 | PGO | -18〜25% | ✅ | **2026-08-23 解放** (ネイティブのみ。再測 2026-08-23: bootphase 交互 5 周全勝、中央値 10.58s→8.67s = -18%、別プロセスと CPU を分け合った状態。`tools/build/pgo-build.sh`、訓練 = regress 固定、定規 bootphase は通常ビルドのまま)。2026-08-11〜22 は「運用判断を開発に持ち込まない」で寝かせていた — 議論と復帰条件は [ADR-0009](../adr/0009-pgo-shelved.md) |
...
| **F1d** | **テンプレートJIT (AArch64手書き+dynasmrt、Craneliftなし)** | 185+ MIPS | 🔬**進行中** | a骨格→bロード→cストア/スタック→d TLBインライン→e 定常WL定規→f 8bit語彙+損益再審 (PR #174〜#181)。**指紋2コース (jboot/jcmd) でon/offビット同一**。F1d-f交互3周: ブート off≈on (**互角、3周目は勝ち**) / gcc窓 off 67.5 vs on-main 47 (-30%) vs on-f1d-f 57 (-15%) = **赤字半減**。窓カバレッジ58%→降格入りで51%・平均7.8命令/入場。授業料: JSLOTS 256Kは衝突再焼き359k回で-13 MIPS (2Mへ)。g 密度 ([ADR-0023](../adr/0023-f1d-g-density.md)): cc死储省略+条件インライン化で **jboot -14% (119 MIPS)・gcc窓-2.3%・起動込み全体はJIT最速**。写像の守りは全数照合 (tests/cond_inline.rs)。残: レジスタ常駐化 (保留)・Stage 2ブロック粒度。Stage 3 (wasm) は前倒し済み (#180) |
| **C15** | **core第4ラウンド** ([ADR-0027](../adr/0027-core-round4.md)) — フォールバック税と具現化税 | 5-10% (interp側) | ✅**一巡** | PR1 **StrRep+負キャッシュ** (#194): gcc窓 off +3-4%・on +3% 全勝。PR2 部分materialize: **ワッシュで寝かせ** (タグ exp/core-matkeep — 具現化はOoOの影、判別則4)。PR3 **16bitストリング** (#195): ブート従来経路落ちの53%=66 A5を語彙へ、ブートon全勝-2〜4%。指紋は3本とも値まで不変。残: cmp+jcc融合/CMPSバルク (実測次第)。台帳: [core-roadmap](core-roadmap.md) |
...
| # | 案 | 期待 | 状態 | メモ |
|---|---|---|---|---|
| W1 | **wasm JIT スロット表 → 2-way** (64K組×2道) | 焼き-63% | ✅ | **2026-08-24、PR #225**。DSL の X 課程 12G 命令で 1-way: 焼き692,361/据付28,686 (24.1倍)・カバレッジ19.5% → 2-way: 焼き255,043/据付58,848 (4.3倍)・26.5%。連想度が容量より効く (2 本のホットなブロックが同じ組で追い出し合う形だった)。**秒は交互3周全勝で -2.2% だけ** (197.4s→193.1s) — 焼き直しの税は主犯ではなく、時間はカバレッジの外 (73%) で使われていると判明。定規は `tools/webtest/jit-soak.mjs` (帳簿 + 同一命令数の秒) |
